...
def plot_decision_boundary(model, ax, resolution=100, colors=('b', 'k', 'r')):
    ...
    ax.contour(xrange, yrange, grid.T, 0, linewidths=1, colors='k', label='Decision boundary')

    ax.scatter(model.X[:,0], model.X[:,1], c=model.y, cmap='Set1',s=40)

    # Plot support vectors (non-zero alphas)
    # as circled points (linewidth > 0)
    mask = model.alphas != 0.0
    ax.scatter(model.X[:,0][mask], model.X[:,1][mask], c='#050505', s=50, label='Support vectors')

    return grid, ax

def take_step(i1, i2, model, eps=0.01):
...
def examine_example(i2, model, tol=0.01):
    y2 = model.y[i2]
    alph2 = model.alphas[i2]
    E2 = model.errors[i2]
    r2 = E2 * y2
